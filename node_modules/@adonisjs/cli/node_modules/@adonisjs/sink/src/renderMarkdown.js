...
const css = `.markdown-body hr::after,.markdown-body::after{clear:both}body{box-sizing:border-box;min-width:200px;max-width:980px;margin:0 auto;padding:45px}
...
function getHtml(moduleName, content) {
    return `<html>
  <head>
...
/**
 * Renders the instructions.md markdown file. Ignores any errors occurred during the
 * process.
 */
async function renderMarkdown(mdFilePath, moduleName) {
    try {
        const contents = await fs_extra_1.readFile(mdFilePath, 'utf-8');
        const html = getHtml(moduleName, marked_1.default(contents.trim()));
        const filePath = path_1.join(os_1.tmpdir(), `adonis-${new Date().getTime()}.html`);
        await fs_extra_1.outputFile(filePath, html);
        await open_1.default(filePath, { wait: false });
    }
    catch (error) {
    }
}
